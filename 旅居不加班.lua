...
task.spawn(function()
    while true do
        about:Label("小帅牛逼: " .. os.date("%Y-%m-%d %H:%M:%S"))
        ...
        if springFestivalTime > 0 then
            about:Label(string.format("小王牛逼: %d天%d小时%d分钟%d秒", 
        ...
        if newYearTime > 0 then
            about:Label(string.format("小帅牛逼: %d天%d小时%d分钟%d秒", 
...
about:Slider('操B距离【正常为70】', 'Sliderflag', 70, 0.1, 250, false, function(v)
        game.Workspace.CurrentCamera.FieldOfView = v
end)

about:Textbox("设置78长度", "Gravity", "输入", function(gravity)
    spawn(function()
        while task.wait() do
...
about:Toggle("靠近自动操B(需要拿起武器)", "AutoAttack", false, function(enabled)
    if enabled then
