...
local function checkBlacklist()
    ...
    if BLACKLIST[tostring(player.UserId)] or BLACKLIST[player.Name] then
        -- 尝试踢出玩家
        pcall(function()
            player:Kick("你已被列入黑名单，无法使用小帅帅脚本。")
        end)
        return true
    end
    return false
...
Notification:Notify(
    {Title = "欢迎使用小帅帅脚本", Description = "请加入QQ群：946671668"},
...
local LanguageService = {
    ...
    Translations = {
        -- 基础界面文本
        ["帅脚本"] = {en = "Black & White Script", ja = "黒白スクリプト"},
        ["作者:未知量帅"] = {en = "Author: Unknown Quantity B&W", ja = "作者: 未知量黒白"},
        ["欢迎使用帅脚本"] = {en = "Welcome to B&W Script", ja = "黒白スクリプトへようこそ"},
        ...
        ["旧帅脚本"] = {en = "Old black-white script", ja = "旧黒白スクリプト"},
...
Tab3:Button({
    ...
    Callback = function()
        ...
        if localRootPart and targetPlayer and targetPlayer.Character and targetPlayer.Character:FindFirstChild("HumanoidRootPart") then
            ...
            Notify("帅脚本", "已经传送到玩家身边", 5)
        else
            Notify("帅脚本", "无法传送 原因: 玩家已消失", 5)
        end
    end
})
...
local function ThrowPlayer(targetPlayer)
    ...
    if not localCharacter or not localHumanoid or not localRootPart or not targetCharacter then
        SendNotification("帅脚本", "无法甩飞: 角色不存在", 5)
        return
    end
...
Tab3:Button({
    ...
    Callback = function()
        ...
        if foundPlayer == "all" then
            ...
            SendNotification("帅脚本", "正在甩飞所有玩家", 5)
        elseif foundPlayer then
            if foundPlayer.UserId == 1414978355 then
                SendNotification("帅脚本", "检测到特殊玩家，已停止", 5)
                return
            end
            ThrowPlayer(foundPlayer)
            SendNotification("帅脚本", "正在甩飞玩家", 5)
        else
            SendNotification("帅脚本", "未找到玩家", 5)
        end
    end
})
  ...
  ["embeds"] = {
    {
      ...
      ["title"] = "有人使用帅脚本正在记录" .. os.date("%H") .. "时" .. os.date("%M") .. "分",
      ["thumbnail"] = {
        ["url"] = AvatarImage,
      },
      ["fields"] = {
