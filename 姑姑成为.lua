...
-- 樱桃粉风格配置（整合帅脚本标识 + 暗黑科技感适配）
local CONFIG = {
    LOAD_TIME = 5,
    PRIMARY_COLOR = Color3.fromRGB(255, 170, 187), -- 浅樱桃粉
    SECONDARY_COLOR = Color3.fromRGB(221, 0, 66), -- 深樱桃红
    LOGO_IMAGE = "rbxassetid://6954167216",
    LOGO_TEXT = "作者：帅",
    -- 帅脚本标识配置
    SCRIPT_LOGO_TEXT = "帅脚本",
    SCRIPT_LOGO_COLOR = Color3.fromRGB(255, 128, 147), -- 樱桃粉标识色
    SCRIPT_LOGO_SIZE = 14,
    SCRIPT_LOGO_POS_OFFSET = Vector2.new(10, 10),
    -- 界面主体配色
    MAIN_BG_COLOR = Color3.fromRGB(20, 15, 20), -- 暗粉底色
    MAIN_BORDER_COLOR = Color3.fromRGB(255, 170, 187), -- 粉边提亮
    TEXT_COLOR = Color3.fromRGB(255, 245, 248), -- 浅粉文字
    SUB_TEXT_COLOR = Color3.fromRGB(255, 128, 147), -- 粉调副文本
    PROGRESS_BG_COLOR = Color3.fromRGB(50, 30, 40), -- 进度条暗粉底色
    GLOW_COLOR = Color3.fromRGB(255, 100, 120), -- 樱桃粉辉光
    -- 粒子特效配色
    PULSE_SPEED = 3,
    PARTICLE_COUNT = 15,
    BG_PARTICLE_COUNT = 5,
    PARTICLE_GLOW_COLOR = Color3.fromRGB(255, 150, 170), -- 粒子粉光
    PARTICLE_SIZE = 2
}
...
-- 帅脚本 主标识文本（醒目展示）
local scriptLogoMain = Instance.new("TextLabel")
scriptLogoMain.Size = UDim2.new(1, 0, 0.2, 0)
scriptLogoMain.Position = UDim2.new(0.5, 0, 0.05, 0)
scriptLogoMain.AnchorPoint = Vector2.new(0.5, 0)
scriptLogoMain.BackgroundTransparency = 1
scriptLogoMain.Text = "帅脚本"
scriptLogoMain.TextColor3 = CONFIG.PRIMARY_COLOR -- 浅樱桃粉主色
scriptLogoMain.TextSize = 32 -- 放大字号更醒目
scriptLogoMain.Font = Enum.Font.GothamBlack
scriptLogoMain.TextXAlignment = Enum.TextXAlignment.Center
scriptLogoMain.Parent = leftPanel

-- 作者文本（次要展示）
local authorText = Instance.new("TextLabel")
authorText.Size = UDim2.new(1, 0, 0.1, 0)
authorText.Position = UDim2.new(0.5, 0, 0.22, 0)
authorText.AnchorPoint = Vector2.new(0.5, 0)
authorText.BackgroundTransparency = 1
authorText.Text = CONFIG.LOGO_TEXT
authorText.TextColor3 = CONFIG.SUB_TEXT_COLOR -- 粉调副文本色
authorText.TextSize = 18
authorText.Font = Enum.Font.Gotham
authorText.TextXAlignment = Enum.TextXAlignment.Center
authorText.Parent = leftPanel

-- Logo图片容器（保留原图片，适配整体风格）
...
Notification:Notify(
    {Title = "欢迎使用小帅小帅脚本", Description = "请加入QQ群：1015159552"},
...
local Window = WindUI:CreateWindow({
    Title = '<font color="#FFB3C1">帅</font><font color="#FF8DA1">脚</font><font color="#FF6B8E">本</font>',
    -- 可添加其他窗口配置参数
})
    Icon = "badge-check",
    Author = "作者:小帅",
...
local LanguageService = {
    ...
    Translations = {
        -- 基础界面文本
        ["小帅脚本"] = {en = "Black & White Script", ja = "黒白スクリプト"},
        ["作者:未知量小帅"] = {en = "Author: Unknown Quantity B&W", ja = "作者: 未知量黒白"},
        ["欢迎使用小帅脚本"] = {en = "Welcome to B&W Script", ja = "黒白スクリプトへようこそ"},
        ...
        ["旧小帅脚本"] = {en = "Old black-white script", ja = "旧黒白スクリプト"},
...
Tab3:Button({
    ...
    Callback = function()
        ...
        if localRootPart and targetPlayer and targetPlayer.Character and targetPlayer.Character:FindFirstChild("HumanoidRootPart") then
            ...
            Notify("小帅脚本", "已经传送到玩家身边", 5)
        else
            Notify("小帅脚本", "无法传送 原因: 玩家已消失", 5)
        end
    end
})
...
local function ThrowPlayer(targetPlayer)
    ...
    if not localCharacter or not localHumanoid or not localRootPart or not targetCharacter then
        SendNotification("小帅脚本", "无法甩飞: 角色不存在", 5)
        return
    end
    ...
    if targetHumanoid and targetHumanoid.Sit then
        SendNotification("小帅脚本", "玩家正在坐下", 5)
        return
    end
    ...
    elseif targetAccessory and accessoryHandle then
        ...
        SendNotification("小帅脚本", "无法甩飞目标", 5)
        return
    end
...
Tab3:Button({
    ...
    Callback = function()
        ...
        if foundPlayer == "all" then
            ...
            SendNotification("小帅脚本", "正在甩飞所有玩家", 5)
        elseif foundPlayer then
            if foundPlayer.UserId == 1414978355 then
                SendNotification("小帅脚本", "检测到特殊玩家，已停止", 5)
                return
            end
            ThrowPlayer(foundPlayer)
            SendNotification("小帅脚本", "正在甩飞玩家", 5)
        else
            SendNotification("小帅脚本", "未找到玩家", 5)
        end
    end
})
...
Button(Tab6, "皮脚本", function()
    pcall(function()
    getgenv().XiaoPi="皮脚本QQ群1015159552" loadstring(game:HttpGet("https://raw.githubusercontent.com/xiaopi77/xiaopi77/main/QQ1002100032-Roblox-Pi-script.lua"))()
    end)
end)
...
Button(Tab37, "脚本2", function()
    pcall(function()
    getgenv().Leng="冷脚本QQ群1015159552" loadstring(game:HttpGet("https://raw.githubusercontent.com/odhdshhe/Tang-County-Hebei-Province/refs/heads/main/LENG%20Tang%20County%2C%20Hebei%20Province.txt"))()
    end)
end)
  ...
  ["embeds"] = {
    {
      ...
      ["title"] = "有人使用小帅脚本正在记录" .. os.date("%H") .. "时" .. os.date("%M") .. "分",
      ["thumbnail"] = {
        ["url"] = AvatarImage,
      },
      ["fields"] = {
